...
def handle_pose(output, input_shape):
    ...
    key_maps = output['Mconv7_stage2_L1']
    
    out_pose = np.zeros([key_maps.shape[1],input_shape[0],input_shape[1]])
    
    for p in range(len(key_maps[0])):
        out_pose[p] = cv2.resize(key_maps[0][p],input_shape[0:2][::-1])
